...
class Client(db.Model):
    ...
    # horario de soporte — 'lv'=L-V / 'ls'=L-S / 'ld'=L-D
    support_schedule = db.Column(db.String(5), nullable=True)
    # importe económico del contrato de soporte y horas gratuitas incluidas
    support_amount = db.Column(db.Float, nullable=True)
    support_free_hours = db.Column(db.Float, nullable=True)
...
class SalaryPayment(db.Model):
    """Historial de pagos de salario realizados a un técnico"""
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
...
class Objetivo(db.Model):
    """Objetivos del equipo o personales"""
    id = db.Column(db.Integer, primary_key=True)
...
class PersonalEvent(db.Model):
    """Eventos de calendario personal por usuario"""
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    user = db.relationship('User', backref='personal_events')
    title = db.Column(db.String(200), nullable=False)
    description = db.Column(db.Text, nullable=True)
...
def manage_clients():
    ...
    if action == 'add':
        ...
        support_amount = request.form.get('support_amount', '').strip()
        support_free_hours = request.form.get('support_free_hours', '').strip()
        support_amount_val = float(support_amount) if support_amount else None
        support_free_hours_val = float(support_free_hours) if support_free_hours else None
        ...
        new_client = Client(
            ...
            support_amount=support_amount_val if has_support else None,
            support_free_hours=support_free_hours_val if has_support else None
        )
        ...
        if client:
            ...
            if client.has_support:
                ...
                sa = request.form.get('support_amount', '').strip()
                sfh = request.form.get('support_free_hours', '').strip()
                client.support_amount = float(sa) if sa else None
                client.support_free_hours = float(sfh) if sfh else None
            else:
                client.support_schedule = None
                client.support_amount = None
                client.support_free_hours = None
...
def admin_all_tasks():
    ...
    try:
        ...
        # Add objectives with due_date as calendar events (visually distinct)
        try:
            objetivos = Objetivo.query.filter(Objetivo.due_date != None).all()
            for obj in objetivos:
                status_color = {'pendiente': '#f59e0b', 'en_progreso': '#3b82f6', 'completado': '#22c55e'}.get(obj.status, '#f59e0b')
                events.append({
                    'id': f'obj_{obj.id}',
                    'title': f'🎯 {obj.title}',
                    'start': str(obj.due_date),
                    'allDay': True,
                    'backgroundColor': status_color,
                    'borderColor': '#ffffff',
                    'textColor': '#000000',
                    'classNames': ['objetivo-event'],
                    'extendedProps': {
                        'is_objetivo': True,
                        'objetivo_id': obj.id,
                        'status': obj.status,
                        ...
                        'username': obj.user.username if obj.user else 'Global',
                        'tech_id': None,
                        'tech_name': obj.user.username if obj.user else 'Global',
                        'tech_color': status_color,
                        'service_type': 'Objetivo',
                        'client': '',
                        'client_id': None,
                        'has_attachments': False,
                        'is_remote': False
            ...
            print(f"Error adding objectives to calendar: {e}")
    ...
    try:
        ...
        return jsonify({
            ...
            'data': {
                ...
                'support_amount': client.support_amount,
                'support_free_hours': client.support_free_hours,
...
@login_required
def api_get_techs():
    """Lista de técnicos activos — accesible para admin"""
    if current_user.role != 'admin':
        return jsonify({'success': False, 'msg': 'Sin permisos'}), 403
    techs = User.query.filter_by(role='tech').order_by(User.username).all()
    return jsonify({'success': True, 'techs': [{'id': t.id, 'username': t.username} for t in techs]})
...
def api_set_salario(user_id):
    ...
    old_amount = sal.amount if sal else None
    if sal:
        sal.amount = amount_f
        sal.notes = data.get('notes', sal.notes)
        sal.updated_at = datetime.now()
    else:
        sal = TechSalary(user_id=user_id, amount=amount_f, notes=data.get('notes'))
        db.session.add(sal)
    # Auto-register in salary payment history if amount changed
    if old_amount != amount_f:
        note_text = data.get('notes', '')
        history_note = f"Salario base actualizado: €{amount_f:.2f}"
        if note_text:
...
# SALARY PAYMENTS API
# ══════════════════════════════════════════════════════════
@app.route('/api/salary_payments', methods=['GET'])
@login_required
def api_get_salary_payments():
    if current_user.role != 'admin':
        return jsonify({'success': False, 'msg': 'No autorizado'}), 403
    user_id = request.args.get('user_id', type=int)
    q = SalaryPayment.query
    if user_id:
        q = q.filter_by(user_id=user_id)
    payments = q.order_by(SalaryPayment.payment_date.desc()).all()
    data = [{
        'id': p.id,
        'user_id': p.user_id,
...
@app.route('/api/salary_payments', methods=['POST'])
@login_required
def api_add_salary_payment():
    if current_user.role != 'admin':
        return jsonify({'success': False, 'msg': 'No autorizado'}), 403
    ...
    tech = User.query.get(user_id)
    if not tech:
        return jsonify({'success': False, 'msg': 'Técnico no encontrado'}), 404
    payment_date_str = body.get('payment_date', str(date.today()))
    try:
        pay_date = date.fromisoformat(payment_date_str)
    except:
        pay_date = date.today()
    p = SalaryPayment(
        user_id=user_id,
...
@app.route('/api/salary_payments/<int:pid>', methods=['PUT'])
@login_required
def api_update_salary_payment(pid):
    if current_user.role != 'admin':
        return jsonify({'success': False, 'msg': 'No autorizado'}), 403
    p = SalaryPayment.query.get_or_404(pid)
    body = request.get_json() or {}
    if 'amount' in body:
        p.amount = float(body['amount'])
    if 'payment_date' in body:
        try:
            p.payment_date = date.fromisoformat(body['payment_date'])
        except:
            pass
    if 'notes' in body:
        p.notes = body['notes']
...
@app.route('/api/salary_payments/<int:pid>', methods=['DELETE'])
@login_required
def api_delete_salary_payment(pid):
    if current_user.role != 'admin':
        return jsonify({'success': False, 'msg': 'No autorizado'}), 403
    p = SalaryPayment.query.get_or_404(pid)
    db.session.delete(p)
...
@login_required
def api_get_objetivos():
    if current_user.role == 'admin':
        objs = Objetivo.query.order_by(Objetivo.created_at.desc()).all()
    else:
        objs = Objetivo.query.filter(
            (Objetivo.user_id == current_user.id) | (Objetivo.user_id == None)
        ).order_by(Objetivo.created_at.desc()).all()
    data = [{
        'id': o.id,
        'title': o.title,
        'description': o.description or '',
        'user_id': o.user_id,
        'username': o.user.username if o.user else 'Global',
        'status': o.status,
        'priority': o.priority,
...
def api_add_objetivo():
    ...
    if user_id == '' or user_id == 'null':
        user_id = None
    if user_id and current_user.role != 'admin' and int(user_id) != current_user.id:
        user_id = current_user.id
...
@app.route('/api/objetivos/<int:oid>', methods=['PUT'])
@login_required
def api_update_objetivo(oid):
    o = Objetivo.query.get_or_404(oid)
    if current_user.role != 'admin' and o.user_id != current_user.id:
        return jsonify({'success': False, 'msg': 'No autorizado'}), 403
    body = request.get_json() or {}
    if 'title' in body:
        o.title = body['title']
...
@app.route('/api/objetivos/<int:oid>', methods=['DELETE'])
@login_required
def api_delete_objetivo(oid):
    o = Objetivo.query.get_or_404(oid)
    if current_user.role != 'admin' and o.user_id != current_user.id:
        return jsonify({'success': False, 'msg': 'No autorizado'}), 403
    db.session.delete(o)
...
# ══════════════════════════════════════════════════════════
# PERSONAL CALENDAR (EVENTOS PERSONALES)
# ══════════════════════════════════════════════════════════
@app.route('/api/personal_events', methods=['GET'])
@login_required
def api_get_personal_events():
    events = PersonalEvent.query.filter_by(user_id=current_user.id).all()
    data = [{
        'id': e.id,
        'title': e.title,
        ...
        'color': e.color or '#7c3aed',
        'all_day': e.all_day
    } for e in events]
    return jsonify({'success': True, 'data': data})

@app.route('/api/personal_events', methods=['POST'])
@login_required
def api_add_personal_event():
    body = request.get_json() or {}
    title = body.get('title', '').strip()
    start_date = body.get('start_date', '')
    if not title or not start_date:
        return jsonify({'success': False, 'msg': 'Título y fecha son obligatorios'}), 400
    e = PersonalEvent(
        user_id=current_user.id,
        title=title,
        description=body.get('description', ''),
...
@app.route('/api/personal_events/<int:eid>', methods=['PUT'])
@login_required
def api_update_personal_event(eid):
    e = PersonalEvent.query.get_or_404(eid)
    if e.user_id != current_user.id:
        return jsonify({'success': False, 'msg': 'No autorizado'}), 403
    body = request.get_json() or {}
    if 'title' in body: e.title = body['title']
    if 'description' in body: e.description = body['description']
    if 'start_date' in body: e.start_date = body['start_date']
    if 'start_time' in body: e.start_time = body['start_time'] or None
    if 'end_date' in body: e.end_date = body['end_date'] or e.start_date
    if 'end_time' in body: e.end_time = body['end_time'] or None
    if 'color' in body: e.color = body['color']
    if 'all_day' in body: e.all_day = body['all_day']
    db.session.commit()
    return jsonify({'success': True})

@app.route('/api/personal_events/<int:eid>', methods=['DELETE'])
@login_required
def api_delete_personal_event(eid):
    e = PersonalEvent.query.get_or_404(eid)
...
def api_get_documentos():
    ...
    data = [{
        ...
        'uploaded_by': d.uploader.username if d.uploader else 'Sistema',
...
@login_required
def api_get_documento(did):
    d = Documento.query.get_or_404(did)
    return jsonify({
        'success': True,
        'doc': {
            'id': d.id,
...
def api_update_documento(did):
    ...
    if 'category' in body: d.category = body['category']
    if 'content' in body: d.content = body['content']
...
def initialize_database():
    ...
    with app.app_context():
        ...
        with db.engine.connect() as conn:
            ...
            # --- CLIENT: support economic fields ---
            _run_migration(conn, 'ALTER TABLE client ADD COLUMN support_amount FLOAT', "client.support_amount")
            _run_migration(conn, 'ALTER TABLE client ADD COLUMN support_free_hours FLOAT', "client.support_free_hours")
